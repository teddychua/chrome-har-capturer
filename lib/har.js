...
function fromPage(page) {
    // page timings
    var startedDateTime = new Date(page.originalRequestDate).toISOString();
    ...
    for (var requestId in page.objects) {
        ...
        // Based on Chromium devtools/front_end/network/RequestTimingView.js
        var blockedTime = firstPositive([timing.dnsStart, timing.connectStart, timing.sendStart]) || 0;
        // override blockedTime to 0 if the condition is met. the condition implies that this is the first resource.
        if (timing.dnsStart == 0 && timing.connectStart == 0 && connectTime > 0) {
            blockedTime = 0;
        }
        var sslTime = timingDelta(timing.sslStart, timing.sslEnd);
        // computed timings
        // This is the Chrome DevTools definition of wait or TTFB.
        // Other tools use timing.receiveHeadersStart - timing.sendEnd
        var waitTime = timing.receiveHeadersEnd - timing.sendEnd;
        var totalTime = object.responseFinishedMs - timing.requestTime;
        var receiveTime = totalTime - timing.receiveHeadersEnd;
...
// Based on Chromium /devtools/network_module.js
